      ...
      return (
        <ul className="list-unstyled">
          {tiers.inHouseFeatures.map((feature, index) => {
            ...
            return (
              <React.Fragment>
                <li className={itemClass} key={feature} style={{margin: "auto 0px"}}>{feature}</li>
                <br />
              </React.Fragment>
            )
          })} 
            ...
            {tiers.akuteHealthFeatures.map((feature) => {
              return (
                <React.Fragment>
                  ...
                  <li className="good" key={feature} style={{margin: "auto 0px"}}>{feature}</li>
                  <br />
                  {/* </div> */}
                </React.Fragment>
              )
        ...
        return (
          <ul className="list-unstyled">
            {tiers.othersFeatures.map((feature) => {
              return (
                <React.Fragment>
                  <li className="bad" key={feature} style={{margin: "auto 0px"}}>{feature}</li>
                  <br />
                </React.Fragment>
              )
            })} 
    ...
    return (
      <React.Fragment>
        ...
        {tiers.choiceType.map((_, index) => {
          return (
            <div key={tiers.choiceType[index]} className="col-md-4 col-lg-4 col-sm-12 col-xs-12" style={{margin: "0 auto"}}>
              <Card>
                <CardHeader className="text-center" tag="h5">{tiers.choiceType[index]}</CardHeader>
                <CardBody>
