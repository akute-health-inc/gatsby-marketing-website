    ...
    return (
      <React.Fragment>
        ...
        {tiers.prices.map((_, index) => {
          return (
            <div key={tiers.planNames[index]} className="col-md-4 col-lg-4 col-sm-12 col-xs-12" style={{margin: "0 auto"}}>
              <Card>
                <CardHeader className="text-center" tag="h5">{tiers.planNames[index]}</CardHeader>
                <CardBody>
                  <CardSubtitle tag="h5" style={{fontWeight: "bold", textAlign: "center"}}>{tiers.prices[index]}</CardSubtitle>
                  <br />
                  {getPlan(index)}
                  <EmailForm styles={styles} name={tiers.planNames[index]}>{tiers.ctas[index]}</EmailForm>
                </CardBody>
              </Card>
            </div>
          )
